...
describe('Starting tests for gaurav-app1', function() {
  ...
  it('website title should be gaurav-app1', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('gaurav-app1');
    })
  });

  it('network-name should be gaurav-network1@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('gaurav-network1@0.0.1.bna');
    });
  });

  it('navbar-brand should be gaurav-app1',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('gaurav-app1');
    });
  });
